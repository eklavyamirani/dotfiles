# Linux's package manifest -- the counterpart to dev/Brewfile on macOS.
#
# The split is the same one documented in dev/Brewfile and
# dev/.config/mise/config.toml, and it does not change per OS: mise owns
...
# if it resolves, pin it in dev/.config/mise/config.toml instead. Adding it in
# both places would have two managers racing for the same name on PATH.
#
# Lives at the repository root rather than inside the dev/ stow package, and
# deliberately so: stow deploys what is under dev/ into $HOME, and this is a
# manifest that bootstrap.sh and reapply.sh *apply*, not a config file that
# belongs in a home directory -- the same reason those two scripts sit at the
# root too. Left in dev/ it would have shown up as a stray ~/nix/ directory
# next to ~/.nix-profile, which is exactly the kind of confusion worth
# avoiding. (dev/Brewfile predates that reasoning and does get linked to
# ~/Brewfile; harmless, since nothing reads it there, and moving it now would
# change macOS behaviour for no gain.)
#
# Applied by:
...
# the store that doubles as a GC root) and dev/.zprofile.d/45-nix.zsh puts its
...
# one thing dev/Brewfile needs `./reapply.sh --prune` for.
#
...
#   nix flake update --flake ./nix   # then commit the flake.lock change
{
  ...
  outputs = { self, nixpkgs, nixpkgs-unstable }:
    let
      ...
      # whole file is inert and dev/Brewfile is the manifest instead.
      ...
      packages = forEachSystem (pkgs: unstable: {
        ...
        default = pkgs.buildEnv {
          ...
          paths = with pkgs; [
            ...
            unstable.mise # installs everything in dev/.config/mise/config.toml
